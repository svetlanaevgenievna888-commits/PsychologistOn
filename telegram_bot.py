...
# Настройки Robokassa
ROBOKASSA_MERCHANT_LOGIN = os.getenv("ROBOKASSA_MERCHANT_LOGIN", "demo")  # ID магазина
ROBOKASSA_PASSWORD_1 = os.getenv("ROBOKASSA_PASSWORD_1", "password_1")  # Пароль #1 для генерации подписи
ROBOKASSA_PASSWORD_2 = os.getenv("ROBOKASSA_PASSWORD_2", "password_2")  # Пароль #2 для проверки уведомлений
ROBOKASSA_TEST_MODE = os.getenv("ROBOKASSA_TEST_MODE", "1")  # 1 - тестовый режим, 0 - боевой
...
class RobokassaPayment:
    ...
    def generate_signature(self, out_sum: float, inv_id: int, password: str = None) -> str:
        """
        ...
        signature_string = f"{self.merchant_login}:{out_sum}:{inv_id}:{password}"
        
        # Вычисляем MD5 хэш
        signature = hashlib.md5(signature_string.encode('utf-8')).hexdigest()
        return signature
    
    def verify_signature(self, out_sum: float, inv_id: int, signature: str, password: str = None) -> bool:
        """
    ...
    def generate_payment_url(self, out_sum: float, inv_id: int, description: str, 
                            result_url: str = None, success_url: str = None, 
                            fail_url: str = None, user_id: str = None) -> str:
        """
        Генерация URL для перехода на оплату
        """
        # Генерируем подпись
        signature = self.generate_signature(out_sum, inv_id)
...
class PaymentSystem:
    ...
    def process_robokassa_payment(self, invoice_id: int, out_sum: float, signature: str) -> bool:
        """
        ...
        # Проверяем сумму
        if abs(payment_info["amount"] - out_sum) > 0.01:  # Допускаем небольшую погрешность
            return False
...
def robokassa_result():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            out_sum = request.form.get('OutSum', type=float)
            signature = request.form.get('SignatureValue', '')
        else:
            invoice_id = request.args.get('InvId', type=int)
            out_sum = request.args.get('OutSum', type=float)
