...
# Воронка: шаг 1 — выбор темы (вовлечение)
FUNNEL_TOPIC_KEYBOARD = InlineKeyboardMarkup([
    [InlineKeyboardButton("😌 Тревога и стресс", callback_data="funnel_topic_anxiety")],
    [InlineKeyboardButton("💕 Отношения", callback_data="funnel_topic_relations")],
    [InlineKeyboardButton("🎯 Карьера и самооценка", callback_data="funnel_topic_career")],
    [InlineKeyboardButton("💬 Другое", callback_data="funnel_topic_other")],
    [InlineKeyboardButton("➡️ Сразу к тарифам", callback_data="funnel_skip")],
])

# Текст голосового приветствия в воронке (коротко, для TTS)
VOICE_GREETING_TEXT = (
    "Привет! Я Артем Сергеевич Мирный, психолог. Рад вас приветствовать. "
    "Выберите, что для вас важнее всего — нажмите одну из кнопок ниже."
)


def _generate_voice_greeting() -> Optional[str]:
    """Генерирует голосовое приветствие (TTS), возвращает путь к временному файлу или None."""
    if not _GTTS_AVAILABLE:
        return None
    try:
        tts = gTTS(text=VOICE_GREETING_TEXT, lang="ru", slow=False)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Воронка: приветствие → выбор темы → ценность → тарифы → оплата."""
    if not update.effective_user or not update.message:
        return
    user_id = str(update.effective_user.id)
    username = update.effective_user.first_name or "Пользователь"
    print(f"[БОТ] Получен /start от user_id={user_id}", flush=True)
    
    if assistant.start_session(user_id):
        await update.message.reply_text(
            f"Добро пожаловать обратно, {username}! 👋\n\n"
            f"У вас есть активная сессия. Можете продолжать общение.\n\n"
            f"/new — новая консультация\n"
            f"/exit — выход"
        )
        return
    
    free_messages[user_id] = 0
    user_states[user_id] = {"funnel": "topic"}
    # Короткое текстовое сообщение
    try:
        await update.message.reply_text("Привет! 👋 Секунду, подгружаю варианты...")
    except Exception:
        pass
    # Голосовое приветствие (TTS) — в отдельном потоке, чтобы не блокировать
    voice_path = await asyncio.to_thread(_generate_voice_greeting)
    if voice_path:
        try:
            with open(voice_path, "rb") as f:
                await update.message.reply_voice(voice=f)
            print(f"[БОТ] Голосовое приветствие отправлено user_id={user_id}", flush=True)
        except Exception as e:
            print(f"[БОТ] Ошибка отправки голоса: {e}", flush=True)
        try:
            os.unlink(voice_path)
        except Exception:
            pass
    start_text = (
        f"Я Артем Сергеевич Мирный — психолог с 15+ летним опытом. "
        f"Работаю в Telegram: удобно и конфиденциально.\n\n"
        f"Что сейчас важнее всего? Нажми кнопку 👇"
    )
    try:
        await update.message.reply_text(start_text, reply_markup=FUNNEL_TOPIC_KEYBOARD)
        print(f"[БОТ] Ответ на /start отправлен user_id={user_id}", flush=True)
    except Exception as e:
        print(f"[БОТ] Ошибка отправки ответа на /start: {e}", flush=True)
        try:
            await context.bot.send_message(chat_id=update.effective_chat.id, text=start_text, reply_markup=FUNNEL_TOPIC_KEYBOARD)
        except Exception as e2:
            print(f"[БОТ] Fallback отправка не удалась: {e2}", flush=True)
...
# Тексты для шага 2 воронки (по теме)
FUNNEL_VALUE_BY_TOPIC = {
    "funnel_topic_anxiety": "С тревогой и стрессом ко мне обращаются чаще всего — и в большинстве случаев уже после первой сессии люди отмечают облегчение. Ты сделал важный шаг, что здесь.",
    "funnel_topic_relations": "Отношения с партнёром, семьёй или коллегами часто упираются в одни и те же узлы. Разобрать их с психологом — самый быстрый путь к изменениям.",
    "funnel_topic_career": "Карьера и самооценка сильно связаны. Работа над этим даёт не только ясность, но и реальные сдвиги в решениях и результатах.",
    "funnel_topic_other": "Понимаю, у каждого своя история. Я работаю с самыми разными запросами — главное, что ты уже здесь.",
    "funnel_skip": "",
}


async def funnel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Воронка: после выбора темы — ценность + тарифы. Сразу answer() — иначе «Загрузка…» не пропадает."""
    ...
    if not data.startswith("funnel_"):
        return
    if user_id in user_states and user_states[user_id].get("funnel") == "topic":
        del user_states[user_id]
    value_text = FUNNEL_VALUE_BY_TOPIC.get(data, "")
    step2_text = (
        f"{value_text}\n\n" if value_text else ""
    ) + (
        "За последний год более 500 человек прошли консультации — многие начинают с 1 часа, чтобы попробовать.\n\n"
        "Выбери формат ниже. После оплаты можешь писать сразу — без записи и ожидания. "
        "Оплата защищена, данные не передаются третьим лицам.\n\n"
        "👇 Выбери тариф и перейди к оплате:"
    )
    try:
        if query.message:
            await query.message.reply_text(step2_text, reply_markup=_tariff_keyboard())
            print(f"[БОТ] Ответ воронки (тарифы) отправлен user_id={user_id}", flush=True)
    except Exception as e:
        print(f"[БОТ] Ошибка отправки воронки: {e}", flush=True)


async def tariff_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка выбора тарифа. Сразу answer() — затем create_payment в потоке, чтобы не блокировать другие нажатия."""
    query = update.callback_query
    if not query:
        return
    try:
    ...
    if not update.effective_user:
        return
    user_id = str(update.effective_user.id)
    tariff_id = (query.data or "").strip()
    print(f"[БОТ] Нажат тариф: {tariff_id!r}", flush=True)
    ...
    if user_id in free_messages:
        del free_messages[user_id]
    try:
        # Блокирующий create_payment в отдельном потоке — иначе event loop занят и другие кнопки «висят»
        payment_url, invoice_id = await asyncio.to_thread(
            assistant.payment_system.create_payment,
            user_id, amount, duration_seconds, description,
        )
        ...
        payment_message = (
            ...
            f"Оплата безопасная, данные защищены. После оплаты можешь писать сразу — без ожидания и записи.\n\n"
            f"Нажми кнопку ниже и заверши оплату за пару минут 👇"
        )
        if query.message:
            try:
                await query.message.reply_text(payment_message, reply_markup=keyboard)
                print(f"[БОТ] Сообщение об оплате отправлено user_id={user_id} tariff={tariff_id}", flush=True)
            except Exception as send_err:
                print(f"[БОТ] Ошибка отправки (возможно длинный URL кнопки): {send_err}", flush=True)
                fallback_msg = payment_message + f"\n\nСсылка для оплаты:\n{payment_url}"
                await query.message.reply_text(fallback_msg[:4000])
    except Exception as e:
        print(f"[БОТ] Ошибка при создании платежа ({tariff_id}): {e}", flush=True)
        try:
            if query.message:
                await query.message.reply_text(
                    f"❌ Ошибка при создании платежа. Попробуйте ещё раз или /start."
                )
        except Exception:
            pass
    finally:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"[БОТ] Сообщение от user_id={user_id}: {text[:50]}...", flush=True)
    
    # Воронка: если на шаге «тема» написали текст — сразу показываем тарифы (не теряем лид)
    if user_id in user_states and isinstance(user_states[user_id], dict) and user_states[user_id].get("funnel") == "topic":
        del user_states[user_id]
        step2_text = (
            "Спасибо, что написал. Чтобы подобрать формат и перейти к оплате — выбери тариф ниже. "
            "После оплаты можно писать сразу.\n\n👇 Выбери тариф:"
        )
        await update.message.reply_text(step2_text, reply_markup=_tariff_keyboard())
        return
    ...
    if not has_active_session:
        ...
        if assistant.payment_system.has_payment_history(user_id):
            ...
            return
        # Новый пользователь: на возражения («подумаю», «дорого») — мягкий возврат к тарифам
        objection_words = ("подумаю", "дорого", "позже", "не уверен", "нет денег", "не уверена", "потом", "скоро")
        if text.lower() in objection_words or any(w in text.lower() for w in objection_words):
            nudge = (
                "Понимаю. Многие начинают с 1 часа — 2 999 ₽, чтобы попробовать без обязательств. "
                "После оплаты можно писать в любое время в течение часа.\n\n"
                "Если удобно — выбери тариф ниже 👇"
            )
            await update.message.reply_text(nudge, reply_markup=_tariff_keyboard())
            return
...
def delete_webhook_for_polling() -> bool:
    """
    Удаляем webhook у бота. Если webhook установлен, Telegram не шлёт обновления в getUpdates (polling).
    """
    try:
        # drop_pending_updates=true — очистить очередь, чтобы не обрабатывать старые сообщения
        r = requests.get(
        ...
        data = r.json() if r.ok else {}
        if data.get("ok"):
            print("✓ Webhook снят — бот будет получать сообщения в Telegram")
            return True
        print(f"⚠ deleteWebhook: {data.get('description', r.text)}")
        return False
    except Exception as e:
        print(f"⚠ Ошибка снятия webhook: {e}")
        return False
...
def run_bot():
    ...
    async def _post_init_remove_webhook(app: Application) -> None:
        """При старте бота снимаем webhook — иначе polling не получает обновления."""
        try:
            await app.bot.delete_webhook(drop_pending_updates=True)
            print("✓ Webhook снят (post_init) — бот готов получать сообщения", flush=True)
        except Exception as e:
            print(f"⚠ post_init delete_webhook: {e}", flush=True)
    ...
    try:
        ...
        application = (
            ...
            .read_timeout(30.0)
            .write_timeout(30.0)
            .post_init(_post_init_remove_webhook)
            .build()
        )
        ...
        application.add_handler(CallbackQueryHandler(funnel_callback, pattern=r"^funnel_"))
        ...
        # Без этого бот не получает сообщения: при установленном webhook getUpdates пустой
        print(">>> Снимаем webhook (обязательно для polling)...")
        sys.stdout.flush()
