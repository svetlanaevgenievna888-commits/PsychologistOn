...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """По /start сразу показываем тарифы и кнопки оплаты."""
    ...
    # Сразу тарифы и кнопки оплаты
    start_text = (
        f"Привет, {username}! 👋\n\n"
        f"Я Артем Сергеевич Мирный — психолог с 15-летним опытом. "
        f"Консультации проходят здесь, в Telegram: после оплаты ты получаешь доступ на выбранное время.\n\n"
        f"Выбери тариф и нажми на кнопку — перейдёшь к безопасной оплате. После оплаты сразу можно писать и начинать консультацию.\n\n"
        f"⏰ 1 час — 2 999 ₽\n"
        f"📅 1 день — 5 999 ₽\n"
        f"📆 Неделя — 9 999 ₽ (популярный)\n"
        f"🗓️ Месяц — 25 999 ₽\n"
        f"🎯 2 месяца — 49 999 ₽\n\n"
        f"Нажми на кнопку ниже, чтобы выбрать тариф и перейти к оплате."
    )
    await update.message.reply_text(start_text, reply_markup=_tariff_keyboard())
...
async def tariff_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        payment_message = (
            ...
            f"После оплаты мы сразу продолжим работу. Нажми на кнопку ниже, чтобы перейти к оплате."
        )
        if query.message:
