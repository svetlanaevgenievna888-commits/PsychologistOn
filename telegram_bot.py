...
def delete_webhook_for_polling() -> bool:
    """
    Удаляем webhook у бота. Если webhook был установлен, Telegram не отдаёт обновления в getUpdates (polling).
    Без вызова deleteWebhook бот в Telegram не будет получать сообщения при использовании polling.
    """
    try:
        r = requests.get(
        ...
        data = r.json() if r.ok else {}
        if data.get("ok"):
            print("✓ Webhook снят — polling будет получать обновления")
            return True
...
def run_bot():
    ...
    try:
        ...
        # Обязательно снимаем webhook — иначе Telegram не шлёт обновления в getUpdates (polling)
