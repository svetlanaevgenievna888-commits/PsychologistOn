...
def run_flask_server():
    """Запуск Flask в отдельном потоке для health check и Robokassa."""
    import sys
    sys.stdout.flush()
    # host='0.0.0.0' обязателен для доступа снаружи контейнера
    app.run(host='0.0.0.0', port=HTTP_PORT, debug=False, use_reloader=False, threaded=True)


def main():
    """Бот запускается в главном потоке (стабильно для PTB 22), Flask — в фоне для health check."""
    import sys
    sys.stdout.flush()
    ...
    if TELEGRAM_BOT_TOKEN:
        print(f"✓ Токен: {TELEGRAM_BOT_TOKEN[:10]}...{TELEGRAM_BOT_TOKEN[-5:]}")
    print(f"✓ GIGACHAT_API_KEY: {'Да' if GIGACHAT_API_KEY else 'Нет'}")
    print(f"✓ ROBOKASSA: {'Да' if (ROBOKASSA_PASSWORD_1 and ROBOKASSA_PASSWORD_2) else 'Нет'}")
    print("=" * 60)
    sys.stdout.flush()

    # Сначала запускаем Flask в фоне — для health check и Robokassa
    flask_thread = threading.Thread(target=run_flask_server, daemon=False, name="FlaskThread")
    flask_thread.start()
    print(f"✓ Flask запущен в фоне на 0.0.0.0:{HTTP_PORT}")
    sys.stdout.flush()
    time.sleep(1)  # даём Flask привязаться к порту

    if not TELEGRAM_BOT_TOKEN:
        print("⚠ Telegram бот не запущен (токен не задан). Работает только HTTP.")
        sys.stdout.flush()
        flask_thread.join()
        return

    # Бот в главном потоке — так надёжнее для python-telegram-bot 22 (asyncio/polling)
    print("✓ Запуск Telegram бота в главном потоке...")
    sys.stdout.flush()
    run_bot()
