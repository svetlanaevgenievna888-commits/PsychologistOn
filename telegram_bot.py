...
async def funnel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Воронка: после выбора темы — ценность + тарифы + срочность."""
...
async def tariff_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка выбора тарифа. Сначала всегда отвечаем на callback — иначе у пользователя висит «Загрузка…»."""
    query = update.callback_query
    if not query or not update.effective_user:
        return
    # Сразу снимаем «Загрузка…» — иначе при любой ошибке ниже клиент так и остаётся в загрузке
    try:
        await query.answer()
    except Exception:
        pass
