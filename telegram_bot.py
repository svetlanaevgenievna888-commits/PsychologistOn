...
def run_bot():
    ...
    try:
        ...
        # Бот в главном потоке — run_polling создаёт event loop и принимает обновления
        print("🚀 Запуск polling (бот в Telegram)...")
        sys.stdout.flush()
        application.run_polling(
            allowed_updates=Update.ALL_TYPES,
            drop_pending_updates=True,  # не обрабатывать старые сообщения после перезапуска
            close_loop=False,
            stop_signals=None
        )
...
def main():
    ...
    # 1. Запускаем Flask в фоне — чтобы health check и Robokassa работали
    flask_thread = threading.Thread(target=run_flask_in_thread, daemon=False, name="FlaskThread")
    flask_thread.start()
    time.sleep(2)  # даём Flask привязаться к порту до проверки здоровья
    print("✓ Flask запущен в фоне на 0.0.0.0:{}".format(HTTP_PORT))
    sys.stdout.flush()

    # 2. Бот в главном потоке — так python-telegram-bot работает стабильно (asyncio в main thread)
    if not TELEGRAM_BOT_TOKEN:
        print("⚠ Токен не задан. Работает только HTTP. Задайте BOT_TOKEN или TELEGRAM_BOT_TOKEN.")
        sys.stdout.flush()
        flask_thread.join()
        return
