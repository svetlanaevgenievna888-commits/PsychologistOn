        ...
        if assistant.payment_system.process_robokassa_payment(invoice_id, out_sum, signature):
            ...
            if user_id:
                try:
                    ...
                    if application is not None and bot_loop is not None:
...
def run_bot():
    ...
    try:
        ...
        # Запускаем polling - run_polling создаст свой event loop, но мы уже установили наш
        # Это гарантирует, что все будет работать в правильном потоке
        application.run_polling(
            allowed_updates=Update.ALL_TYPES,
            close_loop=False,
            stop_signals=None  # Отключаем обработку сигналов, чтобы поток не завершался
        )
...
def main():
    ...
    # Flask будет работать в основном потоке, бот - в отдельном
    ...
    # Используем threaded=False для стабильности healthcheck
