...
# GigaChat API ключ (держите в переменных окружения)
...
if not ROBOKASSA_PASSWORD_1 or not ROBOKASSA_PASSWORD_2:
    raise RuntimeError("Не заданы ROBOKASSA_PASSWORD_1/ROBOKASSA_PASSWORD_2 в переменных окружения.")
...
def start_command(message):
    ...
    if assistant.start_session(user_id):
        ...
        # Инициализируем счетчик бесплатных сообщений
        free_messages[user_id] = 0
        
        # Теплое приветствие с установлением доверия
        welcome_text = f"""Привет, {username}! 

Меня зовут Артем Сергеевич Мирный. Я психолог с 15-летним опытом работы.

Вижу, что ты обратился ко мне. Это уже важный шаг - признать, что нужна поддержка. Я здесь, чтобы помочь тебе разобраться в том, что тебя беспокоит.

Расскажи, что привело тебя ко мне? Что сейчас происходит в твоей жизни, что тебя тревожит или беспокоит?

Не стесняйся, пиши открыто. Я выслушаю тебя без осуждения и помогу найти решение."""
...
def new_session_command(message):
    ...
    bot.reply_to(message, "Хорошо, начинаем новую консультацию. Расскажи, что тебя беспокоит?")
...
def exit_command(message):
    ...
    if user_id in free_messages:
        del free_messages[user_id]
    bot.reply_to(message, "Спасибо за обращение! Если понадобится помощь - я всегда здесь. Береги себя! 🙏")
...
def tariff_callback(call):
    ...
    # Сбрасываем счетчик бесплатных сообщений при выборе тарифа
    if user_id in free_messages:
        del free_messages[user_id]
    ...
    try:
        ...
        # Улучшенный текст с созданием срочности и ценности
        payment_message = f"""Отлично! Ты выбрал: {description}

Сумма к оплате: {amount:,.0f} ₽
Номер счета: {invoice_id}

После оплаты мы сразу продолжим работу над твоей ситуацией. Я буду доступен для тебя в течение всего оплаченного времени.

Нажми на кнопку ниже, чтобы перейти к безопасной оплате. Это займет всего минуту."""
        
        bot.send_message(call.message.chat.id, payment_message, reply_markup=markup)
...
def handle_message(message):
    ...
    if user_id in user_states and isinstance(user_states[user_id], dict) and user_states[user_id].get("state") == "waiting_promo":
        ...
        if assistant.payment_system.process_payment_promo(user_id, text, amount, duration_seconds):
            # Сбрасываем счетчик бесплатных сообщений при успешной оплате
            if user_id in free_messages:
                del free_messages[user_id]
            bot.reply_to(message,
    ...
    # Если нет активной сессии, проверяем бесплатные сообщения
    if not has_active_session:
        ...
        # Если превышен лимит бесплатных сообщений - предлагаем оплату
        if free_messages[user_id] > MAX_FREE_MESSAGES:
            # Предлагаем оплату с мягким переходом
            offer_payment(message, user_id)
            return
    ...
    try:
        ...
        # Задержка зависит от длины ответа: минимум 1.5 секунды, максимум 6 секунд
        ...
        # Если это было последнее бесплатное сообщение, мягко намекаем на продолжение
        if not has_active_session and free_messages.get(user_id, 0) == MAX_FREE_MESSAGES:
            time.sleep(2)
            bot.send_message(message.chat.id,
                "Я вижу, что наш разговор зашел вглубь, и тебе действительно нужна помощь. "
...
def offer_payment(message, user_id):
    """Предложение оплаты с использованием техник продаж"""
    username = message.from_user.first_name or "Пользователь"
    
    # Продающий текст с социальным доказательством и выгодами
    payment_text = f"""{username}, я вижу, что тебе действительно нужна помощь, и я готов продолжить работу с тобой.

За годы практики я помог более 500 людям справиться с похожими ситуациями. Мои клиенты отмечают, что уже после первых часов работы они чувствуют облегчение и видят пути решения своих проблем.

Чтобы продолжить нашу работу и дать тебе полноценную поддержку, мне нужно зарезервировать время специально для тебя. Это позволяет мне полностью сосредоточиться на твоей ситуации.

Вот что я предлагаю:

⏰ 1 час консультации — 2 999 ₽
Для решения конкретного вопроса или первой проработки проблемы

📅 1 день консультации — 5 999 ₽  
Полный день поддержки, когда можешь писать в любое время

📆 Неделя консультации — 9 999 ₽
Неделя непрерывной работы над ситуацией (самый популярный вариант)

🗓️ Месяц консультации — 25 999 ₽
Глубокая проработка с максимальной выгодой

🎯 2 месяца консультации — 49 999 ₽
Долгосрочная поддержка для серьезных изменений

Выбери вариант, который тебе подходит. После оплаты мы сразу продолжим работу."""
    ...
    btn_1w = types.InlineKeyboardButton("📆 Неделя — 9 999 ₽ (популярный)", callback_data="tariff_1w")
        ...
        if assistant.payment_system.process_robokassa_payment(invoice_id, out_sum, signature):
            ...
            if user_id:
                try:
                    ...
                    bot.send_message(
                        int(user_id),
                        f"""Спасибо за доверие, {payment_info.get('user_id', 'друг')}!
